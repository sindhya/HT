...
void form_tour(struct edgeHashTable *edgeHash,struct tourVertex *tourVertices,struct edge *tourEdges)
{
	//Forming Tour
	int curVertex,temp,i,index;
...
//For the last edge which completes the tour
   ...
   if((found1=edgeHashSearch(edgeHash,first_edge))==NULL)
	...
	weight=numTypeB=numTypeA=0;
        ...
        {   
                weight+=tourEdges[index].weight;
                if(tourEdges[index].type)
                        numTypeB++;
                else
                        numTypeA++;
        }   
...
int isAdjacent(struct tourVertex *vertex1,int i)
{
...
void optimize_tour(struct edgeHashTable *edgeHash,struct edge* tourEdges,struct tourVertex* tourVertices)
{
...
/*	printf("\ntour vertices,,,:");
	...
	for(i=1;i<=vertexId;i++)
	{
		...
		while(j!=1)
		{
			if(!isAdjacent(&tourVertices[i],j))
			{
	...
	printf("\n-------------------------");
	form_tour(edgeHash,tourVertices,tourEdges);
	printf("\nWeight Optimization:");
	printf("\n--------------------");
	optimize_weight(edgeHash,tourEdges,tourVertices);
}


void optimize_weight(struct edgeHashTable *edgeHash,struct edge* tourEdges,struct tourVertex* tourVertices)
{
		...
		while(j!=1)
		{
			if(!isAdjacent(&tourVertices[i],j))
			{
				...
				else if(found1&&found2)
				{
					if(foundcross1&&foundcross2)
					{
						weight1=found1->weight;weight1+=found2->weight;
						weight2=foundcross1->weight;weight2+=foundcross2->weight;
						if(weight2<weight1)
						{
